...
class EasyApplyBot:
    ...
    # max search time is 10 hours, adjust to preference
    ...
    def browser_options(self):
        ...
        # Disabling webdriver flags to avoid bot detection
    ...
    def start_linkedin(self, username, password):
        print("Initializing log in....Please wait.")
    ...
    def applications_loop(self, position, location):
        ...
        log.info("Searching for jobs... Please wait..")
        ...
        while time.time() - start_time < self.MAX_SEARCH_TIME:
            try:
                ...
                # LinkedIn search results are a scrollable <div>, so scrolling to bottom
                scrollresults = self.browser.find_element_by_class_name("jobs-search-results")
                
                # selenium only scrapes visible results, need for page to load
                ...
                # collecting job IDs of each position's link
                IDs = []
                ...
                # cleaning out jobs already applied for
                before = len(IDs)
                jobIDs = [x for x in IDs if x not in self.appliedJobIDs]
                after = len(jobIDs)

                # assuming 25 jobs per browser window
                ...
                # looping through job IDs to apply
                for i, jobID in enumerate(jobIDs):
                    count_job += 1
                    self.get_job_page(jobID)

                    # locates easy apply button
                    button = self.get_easy_apply_button()
                    
                    # filter to skip positions blacklisted
                    if button is not False:
                        ...
                        log.info("This button does not exist. No Easy Apply Button located.")
                        result = False
                    ...
                    # bot set to sleep every 16 apps
                    if count_application != 0 and count_application % 12 == 0:
                        sleepTime = random.randint(500, 900)
                        log.info(f"""********count_application: {count_application}************\n\n
                                    Time for a break - see you in:{int(sleepTime / 60)} min
                                ****************************************\n\n""")
                        time.sleep(sleepTime)


                    # if done with this page's positions, go to next page
    ...
    def send_resume(self):
        ...
        try:
            ...
            while True:
                # uploading cover letter if option to
                ...
                # clicks next or submit button if possible
                button = None
                buttons = [next_locater, review_locater, follow_locator, submit_locater, submit_application_locator]
            ...
            log.info(e)
            log.info("Sorry, unable to apply to this position.")
            raise (e)
    ...
    def next_jobs_page(self, position, location, jobs_per_page):
        self.browser.get("https://www.linkedin.com/jobs/search/?f_LF=f_AL&keywords=" + position + location + "&start=" + str(jobs_per_page))
        self.avoid_lock()
    ...
    def finish_apply(self):
        self.browser.close()
        print("Finished applying, program shutting down...")
...
if __name__ == '__main__':
    ...
    print("Initiating automated LinkedIn applications...")
